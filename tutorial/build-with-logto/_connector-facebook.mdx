...
5. In the Facebook Login Settings page, fill `${your_fastlogs_origin}/callback/${connector_id}` in the "Valid OAuth Redirect URIs" field. The `connector_id` can be found on the top bar of the Fastlogs Admin Console connector details page. E.g.:
   - `https://your-fastlogs-domain.com/callback/${connector_id}` for production
...
7. In the Facebook app dashboard page, click the sidebar -> "Settings" -> "Basic" and then you will get the **App ID** and **App secret**. The _App ID_ is the `clientId` in your Fastlogs connector conifg and the _App secret_ is the `clientSecret`.
