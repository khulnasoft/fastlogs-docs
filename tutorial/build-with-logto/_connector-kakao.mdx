...
- Add below URL into `Redirect URI`
  - `http(s)://YOUR_URL/callback/${connector_id}` (The `connector_id` can be found on the top bar of the Fastlogs Admin Console connector details page.)
  - (Please replace `YOUR_URL` with your `Fastlogs` URL, and choose `http` or `https` on your situation.)
