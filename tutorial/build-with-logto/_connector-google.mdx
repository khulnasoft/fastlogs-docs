...
- Click **+ Add URI** to add an authorized domain to the **Authorized JavaScript origins** section. This is the domain that your fastlogs authorization page will be served from. In our case, this will be `${your_fastlogs_origin}`. e.g.`https://fastlogs.dev`.
- Click **+ Add URI** in the \***\*Authorized redirect URIs\*\*** section to set up the \***\*Authorized redirect URIs\*\***, which redirect the user to the application after logging in. In our case, this will be `${your_fastlogs_endpoint}/callback/${connector_id}`. e.g. `https://fastlogs.dev/callback/${connector_id}`. The `connector_id` can be found on the top bar of the Fastlogs Admin Console connector details page.
