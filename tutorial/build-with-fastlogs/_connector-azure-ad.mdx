...
- Click **New Registration** at the top and enter a description, select your **access type** and add your **Redirect URI**, which redirect the user to the application after logging in. In our case, this will be `${your_fastlogs_origin}/callback/${connector_id}`. e.g. `https://fastlogs.dev/callback/${connector_id}`. You need to select Web as Platform. The `connector_id` can be found on the top bar of the Fastlogs Admin Console connector details page.
...
- Add your App Registration's **Client ID** into fastlogs json.
- Add your **Client Secret** into fastlogs json.
- Add your App Registration's **Tenant ID** into fastlogs json.
- Add your Microsoft **Login Url** into fastlogs json. This defaults to "https://login.microsoftonline.com/" for many applications, but you can set your custom domain if you have one. (Don't forget the trailing slash)
