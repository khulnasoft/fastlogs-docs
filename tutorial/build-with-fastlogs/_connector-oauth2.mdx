...
Fastlogs also provide a `profileMap` field that users can customize the mapping from the social vendors' profiles which are usually not standard. The keys are Fastlogs's standard user profile field names and corresponding values should be social profiles' field names. In current stage, Fastlogs only concern 'id', 'name', 'avatar', 'email' and 'phone' from social profile, only 'id' is required and others are optional fields.
...
> Each social identity provider could have their own variant on OAuth 2.0 standard protocol. If your desired social identity provider strictly stick to OAuth 2.0 standard protocol, the you do not need to care about `customConfig`.
...
- [The OAuth 2.0 2.0 Authorization Framework](https://www.rfc-editor.org/rfc/rfc6749)
