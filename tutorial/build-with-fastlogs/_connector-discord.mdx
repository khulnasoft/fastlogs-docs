...
- Click the **New Application** button to create an application, choose a name for it (Ex: FastlogsAuth), tick the box and click **Create**.
- Go to **OAuth2** page and click **Reset Secret**
- Take note of the **CLIENT ID** and **CLIENT SECRET** fields
- Add the valid redirects (Ex: **`http://auth.mycompany.io/callback/${connector_id}`**). The `connector_id` can be found on the top bar of the Fastlogs Admin Console connector details page.
