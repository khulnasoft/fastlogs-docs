...
Fastlogs core calls the Amazon Simple Email Service APIs via this connector, with the help of which Fastlogs end-users can register and sign in to their Fastlogs account via email verification code.
...
2. Complete the settings of the `Amazon Simple Email Service` connector:
   ...
   - `emailAddress`: The email address you use to send mail, in the format of `Fastlogs<noreply@fastlogs-docs.khulnasoft.com>` or `<noreply@fastlogs-docs.khulnasoft.com>`
...
That's it. Don't forget to [Enable connector in sign-in experience](https://docs.fastlogs-docs.khulnasoft.com/docs/tutorials/get-started/passwordless-sign-in-by-adding-connectors#enable-sms-or-email-passwordless-sign-in).
