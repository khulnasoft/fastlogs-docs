...
- For `서비스 URL(Service URL)`, type `http(s)://YOUR_URL` (ex. https://fastlogs-docs.khulnasoft.com)
- For `네이버 로그인(Naver Login) Callback URL`, type `http(s)://YOUR_URL/callback/connector_id` (e.g. https://fastlogs-docs.khulnasoft.com/callback/connector_id)
...
> ⚠️ **Caution**
>
> The `connector_id` can be found on the top bar of the Fastlogs Admin Console connector details page.
