...
Make sure to [register your api resource in Fastlogs Admin Console](./README.mdx#register-the-api-resources-using-fastlogs-admin-console) before continuing.
...
Pick the cryptography your are using in Fastlogs. (`ecdsa` by default)
...
def requires_auth(f):
  ...
  def decorated(*args, **kwargs):
    ...
    # jwks_uri endpoint retrieved from Fastlogs
    jwks_uri = urlopen('https://<your-fastlogs-domain>/oidc/jwks')

    # issuer retrieved from Fastlogs
    ...
    try:
      payload = jwt.decode(
        ...
        # The API's registered resource indicator in Fastlogs
