...
Fastlogs is an off-the-shelf authorization service that enables you to enforce authorization on your backend [API resources](../../references/resources/README.md) , enhancing their privacy protection.

## Register the API resources

To obtain an audience-restricted `access_token` from Fastlogs, you must register your backend APIs. When an authorization request is received, Fastlogs will identify the registered [API resources](../../references/resources/README.md) and grant access accordingly.

To register your APIs, navigate to the API Resources section in the console. You will see a built-in resource with the API identifier displayed as `https://tenantid.fastlogs.app/api`. This resource encompasses all the management APIs of Fastlogs and ensures that they are protected and accessible only to authorized users of Fastlogs.
...
:::caution
The API Identifier is unique and used as the single source of truth for Fastlogs, and it's **NOT** editable once created. Be careful when you create it.
:::
...
See [API Resource Fastlogs Schema](../../references/resources/README.md#fastlogs-api-resource-schema) for detailed API setting definitions.

:::info
The API Resources registered in Fastlogs Admin Console will be shared across all your applications.
:::
...
Fastlogs issues a standard [JWT](https://datatracker.ietf.org/doc/html/rfc7519) format authorization token for each authorized API request. The token is encrypted and signed as a [JWS](https://datatracker.ietf.org/doc/html/rfc7515) token.
...
A standard schema of Fastlogs issued JWS Payload: (claims may vary, based on your custom OIDC config)
...
3. The token's issuer is `https://<your-fastlogs-domain>/oidc` (issued by your Fastlogs auth server)
4. The token's audience is equal to the current receiver's resource indicator registered in Fastlogs Admin Console
...
Fastlogs uses the code-based OAuth 2.0 Authorization Protocol to make your API request safe. If you are interested in the strategy behind it, refer to OAuth 2.0's [specification](https://datatracker.ietf.org/doc/html/rfc6749#section-1.3.1) for more details.
