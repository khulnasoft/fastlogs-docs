...
A standard schema of Fastlogs JWS Payload: (claims may vary, based on your OIDC config)
...
- the token's issuer is `https://<your-fastlogs-domain>/oidc` (issued by the Fastlogs server)
...
const { payload } = await jwtVerify(
  ...
  publicKey, // jwks inquired from Fastlogs server
  {
    // expected issuer of the token, should be (issued by the Fastlogs server)
...
public class JwtTokenUtil {
    ...
    public Boolean validateToken(String token, String listener, String fastlogsPath) {
        final Claims claims = getAllClaimsFromToken(token);
        final String audience = claims.getAudience();
        final String issuer = claims.getIssuer();

        return (audience.equals(listener) && issuer.equals(fastlogsPath) && !isTokenExpired(claims));
    }
...
class MyFastlogsBearerTokenValidator(JWTBearerTokenValidator):
    """
    jwks: You will need to inquire the jwks from Fastlogs's jwks endpoint: "https://<your-fastlogs-domain>/oidc/jwks" and pass in the response
    """
    def __init__(self, jwks, issuer="https://<your-fastlogs-domain>/oidc", audience):
        public_key = JsonWebKey.import_key_set(jwks)
        super(MyFastlogsBearerTokenValidator, self).__init__(public_key, issuer)
        self.claims_options = {
           'exp': {'essential': True},
    ...
    def validate_token(self, token, scopes, request):
        super(MyFastlogsBearerTokenValidator, self).validate_token(token, scopes, request)
