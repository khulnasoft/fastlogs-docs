...
To gain the specific access, the client needs to exchange a proper Access Token from Fastlogs, and use it as a Bearer Token in the request header.

### Configure client SDK

This tutorial uses TypeScript as the sample language, and the convention applies to all Fastlogs Client SDKs.

When [integrating client SDK](/docs/recipes/integrate-fastlogs/) with your application, make sure to include both `resources` and `scopes` (permissions) in the configuration for `FastlogsClient`.
...
```ts
const accessToken = await fastlogsClient.getAccessToken('https://api-1.store.io');
```
...
```swift
fastlogsClient.getAccessToken("https://api-1.store.io", { fastlogsException, accessToken ->
    //...
})
```
...
- A subset of `scopes` in the initial Fastlogs config that includes all the scopes that belong to the resource indicator you passed in `getAccessToken()`
- The scopes that the user can obtain based on your RBAC configuration

For example, in the initial Fastlogs config:
...
{
  // ...other token claims
  iss: '<your-fastlogs-endpoint>/oidc', // issuer
...
During the auth flow, Fastlogs will try to validate two permissions:
...
Per definition of the cartesian product, during the auth flow, Fastlogs will try to validate four permissions:
...
But the `write:order` permission does not exist in the API Resource `https://api-2.store.io`. Fastlogs will ignore (i.e. "downscope") this permission in the API Resource `https://api-2.store.io` which will reflect in the auth result.

Now you can also specify `https://api-2.store.io` as the resource indicator when calling `fastlogsClient.getAccessToken()`. Only `read:order` will present in the `scope` claim of the returned Access Token even if the user has the full access of `https://api-2.store.io`.
