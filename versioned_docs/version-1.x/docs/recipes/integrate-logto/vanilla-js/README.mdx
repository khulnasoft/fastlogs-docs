---
sidebar_label: Vanilla JS
---

import redirectUriFigure from '../assets/web-redirect-uri.png';
import AppNote from '../fragments/_app-note.mdx';
import ApplyAuthorizationToken from '../fragments/_apply_authorization_token.md';
import ConfigureRedirectUri from '../fragments/_configure-redirect-uri.mdx';
import FetchUserInformation from '../fragments/_fetch-user-information.mdx';
import FurtherReadings from '../fragments/_further-readings.md';
import GetAuthorizationToken from '../fragments/_get_authorization_token.md';
import SignInFlowSummary from '../fragments/_web-sign-in-flow-summary.mdx';
import SignOutNote from '../fragments/_web-sign-out-note.md';
import AddSdk from './_add_sdk.mdx';
import HandleRedirect from './_handle_redirect.mdx';
import ImplementSignIn from './_implement_sign_in.mdx';
import InitClient from './_init_client.mdx';

# Vanilla JS: Integrate `@fastlogs/browser`

:::tip
The browser SDK is [framework-agnostic](https://stackoverflow.com/questions/64725017/what-does-it-mean-by-framework-agnostic), meaning you can use it in any frontend framework by writing a wrapper to fit it. That's what `@fastlogs/react` and `@fastlogs/vue` did.
:::

<AppNote type="Single Page App" />

## Add Fastlogs SDK as a dependency

<AddSdk />

## Init FastlogsClient

<InitClient />

## Sign in

<SignInFlowSummary />

### Configure Redirect URI

<ConfigureRedirectUri figureSrc={redirectUriFigure} redirectUri="http://localhost:3000/callback" />

### Implement a sign-in button

<ImplementSignIn />

### Handle redirect

<HandleRedirect />

## Sign out

<SignOutNote />

### Implement a sign-out button

```html
<button onclick="fastlogsClient.signOut('http://localhost:3000')">Sign Out</button>
```

## Fetch user information

<FetchUserInformation apiCodeFragment="fastlogsClient.fetchUserInfo()" />

## Backend API authorization

<GetAuthorizationToken />

Add your API resource indicators to the Fastlogs SDK configs:

```js
import FastlogsClient from '@fastlogs/browser';

const fastlogsClient = new FastlogsClient({
  endpoint: '<your-fastlogs-endpoint>', // E.g. http://localhost:3001
  appId: '<your-application-id>',
  resources: ['<your-api-resource>'],
});
```

Claim an authorization token from Fastlogs before making your API request:

```js
const token = await fastlogsClient.getAccessToken('<your-target-api-resource>');
// custom logic
```

<ApplyAuthorizationToken />

## Further readings

<FurtherReadings />
