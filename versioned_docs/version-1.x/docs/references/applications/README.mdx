...
In Fastlogs for OIDC, an _Application_ refers to a specific software program or service that is registered with the Fastlogs platform and has been granted authorization to access user information or perform actions on behalf of a user. Applications are used to identify the source of requests made to the Fastlogs API, as well as to manage the authentication and authorization process for users accessing those applications.

The use of applications in Fastlogs's sign-in experience allows users to easily access and manage their authorized applications from a single location, with a consistent and secure authentication process. This helps to streamline the user experience and ensure that only authorized individuals are accessing sensitive information or performing actions on behalf of the organization.

Applications are also used in Fastlogs's audit logs to track user activity and identify any potential security threats or breaches. By associating specific actions with a particular application, Fastlogs can provide detailed insights into how data is being accessed and used, allowing organizations to better manage their security and compliance requirements.
If you want to integrate your application with Fastlogs, see [Integrate Fastlogs in your application](../../recipes/integrate-fastlogs).

## Properties

### Application ID

_Application ID_ is a unique auto-generated key to identify your application in Fastlogs, and is referenced as [client id](https://www.oauth.com/oauth2-servers/client-registration/client-id-secret/) in OAuth 2.0.
...
The _Application Name_ is an important component of managing applications in Fastlogs, as it allows administrators to easily identify and track the activity of individual applications within the platform.
...
_Redirect URIs_ that are a list of valid redirect URIs that have been pre-configured for an application. When a user signs in to Fastlogs and attempts to access the application, they are redirected to one of the allowed URIs specified in the application settings.

The allowed URIs list is used to validate the redirect URI that is included in the authorization request sent by the application to Fastlogs during the authentication process. If the redirect URI specified in the authorization request matches one of the allowed URIs in the application settings, the user is redirected to that URI after successful authentication. If the redirect URI is not on the allowed list, the user will not be redirected and the authentication process will fail.

```
It is important to ensure that all valid redirect URIs are added to the allowed list for an application in Fastlogs, in order to ensure that users can successfully access the application after authentication.
```
...
_Post Sign-out Redirect URIs_ are a list of valid URIs that have been pre-configured for an application to redirect the user after they have signed out from Fastlogs.
...
The _CORS (Cross-Origin Resource Sharing) Allowed Origins_ are a list of permitted origins from which an application can make requests to the Fastlogs service. Any origin that is not included in the allowed list will not be able to make requests to the Fastlogs service.

The CORS Allowed Origins list is used to restrict access to the Fastlogs service from unauthorized domains, and to help prevent cross-site request forgery (CSRF) attacks. By specifying the allowed origins for an application in Fastlogs, the service can ensure that only authorized domains are able to make requests to the service.
...
_Authorization Endpoint_ is an OIDC term, and it is a required endpoint that is used to initiate the authentication process for a user. When a user attempts to access a protected resource or application hat has been registered with the Fastlogs platform, they will be redirected to the _Authorization Endpoint_ to authenticate their identity and obtain authorization to access the requested resource.
...
When enabled, Fastlogs will always issue Refresh Tokens, regardless of whether `prompt=consent` is presented in the authentication request, nor `offline_access` is presented in the scopes.
...
When enabled, Fastlogs will issue a new Refresh Token for token requests under the following conditions:
...
Note: TTL refreshment is unavailable in SPA (Single Page Apps) for security reasons. This means Fastlogs will not extend the TTL through token requests. To enhance the user experience, you can enable the Rotate Refresh Token feature, allowing Fastlogs to issue a new Refresh Token when necessary.
