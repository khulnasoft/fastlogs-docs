...
_Connectors_ play a critical role in Fastlogs. With their help, Fastlogs enables end-users to use passwordless registration or sign-in and the capabilities of signing in with social accounts. With the increasing popularity of websites and applications, passwordless and social sign-ins allow users to avoid managing numerous accounts and passwords.
...
_id_ is an _unique_ string-typed key to identify a connector in Fastlogs.
...
<table class="simple-table">
  <tbody>
    ...
    <tr>
      ...
      <td>N/A</td>
      <td>
        It&#x27;s possible for a user to sign in to a Fastlogs account that was created using another
    ...
    <tr>
      ...
      <td>
        Signing in to Fastlogs using these two different connectors will always create separate Fastlogs
...
:::note
Fastlogs only supports "standard" social connectors. That is to say, all Fastlogs's Sms or Email connectors are NOT "standard".

Fastlogs call connectors built upon open and standard protocols (e.g., OAuth, OIDC, SAML, etc.) as "standard" connectors. Fastlogs's users are expected to construct multiple instances on each standard connector based on this context. For example, suppose that Fastlogs has already provided an OAuth standard connector, users can build "OAuth GitHub connector", "OAuth Google connector" and "OAuth Facebook connector" instances. They are all based on the Fastlogs OAuth standard connector.

If you are familiar with Fastlogs's connector design, at most ONE Sms or Email connector can exist at the same time, which means Fastlogs do not need "standard" Sms or Email connectors at the current stage.
:::
...
If _syncProfile_ is FALSE, the Fastlogs user's basic information (including name and avatar) will be updated only when the user first signs up to Fastlogs via this connector. Otherwise, every time users sign in to Fastlogs through the connector, the Fastlogs account profile will be updated.
...
:::note
In current Fastlogs version, only one _Sms/Email_ connector can exist at the same time, all other connectors with same type are automatically deleted.
