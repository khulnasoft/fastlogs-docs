...
Fastlogs handles environment variables in the following order:
...
:::caution
If you run Fastlogs via `npm start` in the project root, `NODE_ENV` will always be `production`.
:::
...
| ----------------------- | ------------------------------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| NODE_ENV                | `undefined`                          | <code>'production' &#124; 'test' &#124; undefined</code> | What kind of environment that Fastlogs runs in.                                                                                                                                                                             |
| PORT                    | `3001`                               | `number`                                                 | The local port that Fastlogs listens to.                                                                                                                                                                                    |
| ADMIN_PORT              | `3002`                               | `number`                                                 | The local port that Fastlogs Admin Console listens to.                                                                                                                                                                      |
...
| DB_URL                  | N/A                                  | `string`                                                 | The [Postgres DSN](https://www.postgresql.org/docs/14/libpq-connect.html#id-1.7.3.8.3.6) for Fastlogs database.                                                                                                             |
...
In this case, you're likely want to set `TRUST_PROXY_HEADER` to `true` which indicates if proxy header fields should be trusted. Fastlogs will pass the value to [Koa app settings](https://github.com/koajs/koa/blob/master/docs/api/index.md#settings).
...
Managing too many environment variables are not efficient and flexible, so most of our general configs are stored in the database table `fastlogs_configs`.
