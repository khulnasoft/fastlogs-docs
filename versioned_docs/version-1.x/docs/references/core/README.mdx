...
_Core Service_ is a monolith service for critical Fastlogs duties. The source code is in [`/packages/core`](https://github.com/fastlogs-docs.khulnasoft.com/fastlogs/tree/master/packages/core).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>APIs for managing Fastlogs. The admin role is required.</td>
      <td>
        <code>/api</code>
      </td>
    </tr>
...
Backend APIs, including OIDC, are built within the `core` package, while frontend proxies depend on the corresponding sibling packages in the Fastlogs monorepo.

## OIDC Provider

Fastlogs uses the amazing certified [OpenID Connect](https://openid.net/connect/) implementation [`node-oidc-provider`](https://github.com/panva/node-oidc-provider) under the hood. The provider is mounted at `/oidc`, and you can check relative configurations and files in [`packages/core/src/oidc`](https://github.com/fastlogs-docs.khulnasoft.com/fastlogs/tree/master/packages/core/src/oidc).
...
_Management API_ is a set of APIs that manage and update Fastlogs data. Only users with the `admin` role have access to them.
...
Fastlogs has three frontend proxies:
