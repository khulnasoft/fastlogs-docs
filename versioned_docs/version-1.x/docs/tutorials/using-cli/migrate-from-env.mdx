...
:::note
You can skip this page if your Fastlogs version is greater than or equal to `v1.0.0-beta.11`.
:::
...
DB_URL=postgresql://localhost:5432/fastlogs
```
...
```bash
fastlogs db seed oidc
# Or
fastlogs db seed oidc --env /your/path/to/.env
```
...
```bash
npx @fastlogs/cli db seed oidc
# Or
npx @fastlogs/cli db seed oidc --env /your/path/to/.env
```
...
From now on, you can safely remove `OIDC_PRIVATE_KEYS` and `OIDC_COOKIE_KEYS` from the environment variables. Remember to restart your Fastlogs instance to get the changes reflected.
