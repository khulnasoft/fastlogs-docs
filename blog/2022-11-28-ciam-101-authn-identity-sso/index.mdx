...
I began the build Fastlogs because I noticed that Identity and Access Management (IAM) had become increasingly complex and expansive over time. The concept of IAM is even large enough to give rise to new concepts, such as WIAM (Workforce IAM) and CIAM (Customer IAM).
...
When we discuss "Multi-tenant", we are referring to multiple Fastlogs instances that are identity-isolated from one another. In other words, multiple Fastlogs instances.
...
:::note
The definition of "Tenant" varies across CIAM products. Some refer to the term "Organization" in Fastlogs, which will be introduced later.
:::
...
```mermaid
graph LR
  A[iOS App] --> |Invoke sign-in| Fastlogs --> |Redirect| GitHub
  GitHub --> |Return GitHub ID| Fastlogs --> |Return Fastlogs ID| A
```
...
Both have two.
iOS App is a service provider to Fastlogs, while Fastlogs is an identity provider.
Fastlogs is also a service provider to GitHub, while GitHub is an identity provider.
Thus, Fastlogs is a Service Provider also a Identity Provider.
</details>
...
**❓ How can Fastlogs (or a CIAM product) help?**

<details>
<summary>Answer</summary>
    
Create a Fastlogs instance for each business partner. Each partner holds a Tenant. Projects are mapped to "Apps" in Fastlogs.
...
Fastlogs offers a universal sign-in experience (i.e. SSO) within a Tenant, so users don’t need to sign in again when accessing another app in the same Tenant if they already signed in.
</details>
...
Fastlogs is currently focusing on the approach outlined above. Multiple external identity providers, such as a third-party social sign-in provider, may exist independently and without connection.

Despite this, Fastlogs remains the single source of truth for Identities, simply "borrowing" them from other providers. In this case, Fastlogs acts as both an Identity Provider (to GSuite apps) and a Service Provider (to external Identity Providers).
...
In this case, Fastlogs is both an Identity Provider and a Service Provider. It appears that SSO is more complex than a normal sign-in process. What are the benefits for the identity owner?
