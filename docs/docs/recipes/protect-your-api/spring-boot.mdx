...
If you are not ready, [read this](./README.mdx#register-the-api-resources-using-fastlogs-admin-console) before continuing.

:::

Your web application may run on the server-side using Spring Boot framework.
For now, you need to integrate Fastlogs in Spring Boot manually.
...
you DO NOT need to add additional libraries from Fastlogs to integrate.
...
By default, your Fastlogs's issuer and JWKS URI are `https://<your-fastlogs-domain>/oidc` and `https://<your-fastlogs-domain>/oidc/jwks`
(e.g. `http://localhost:3001/oidc` and `http://localhost:3001/oidc/jwks` in the local development environment).

:::note

All the latest Fastlogs Authorization Server Configurations can be found by `https://<your-fastlogs-domain>/oidc/.well-known/openid-configuration`
...
{
  // ...
  "issuer": "https://<your-fastlogs-domain>/oidc",
  "jwks_uri": "https://<your-fastlogs-domain>/oidc/jwks"
  // ...
}
```
...
spring:
  security:
    oauth2:
      resourceserver:
        jwt:
          issuer-uri: <your-fastlogs-issuer-uri> # e.g. http://localhost:3001/oidc
          jwk-set-uri: <your-fastlogs-jwks-uri> # e.g. http://localhost:3001/oidc/jwks
```

- `audience`:
  The unique API identifier (i.e. API indicator) of your protected API resource.
- `spring.security.oauth2.resourceserver.jwt.issuer-uri`:
  The `iss` claim value and the issuer URI in the JWT issued by Fastlogs.
...
```java
// path/to/project/src/main/java/io/fastlogs/springboot/sample/validator/AudienceValidator.java
package io.fastlogs.springboot.sample.validator;
...
```java
// path/to/project/src/main/java/io/fastlogs/springboot/sample/configuration/SecurityConfiguration.java
package io.fastlogs.springboot.sample.configuration;

import com.nimbusds.jose.JOSEObjectType;
import com.nimbusds.jose.proc.DefaultJOSEObjectTypeVerifier;
import com.nimbusds.jose.proc.SecurityContext;
import io.fastlogs.springboot.sample.validator.AudienceValidator;
...
```java
// path/to/project/src/main/java/io/fastlogs/springboot/sample/controller/ProtectedController.java
package io.fastlogs.springboot.sample.controller;
