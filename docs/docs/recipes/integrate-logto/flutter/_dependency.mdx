...
[flutter_web_auth](https://pub.dev/packages/flutter_web_auth) is used behind Fastlogs's flutter SDK. We rely on its webview-based interaction interface to open Fastlogs's authorization pages.
...
In order to capture the callback url from Fastlogs's sign-in web page, you will need to register your sign-in redirectUri to the AndroidManifest.xml.
...
<activity android:name="com.linusu.flutter_web_auth.CallbackActivity" android:exported="false">
    <intent-filter android:label="flutter_web_auth">
        ...
        <data android:scheme="io.fastlogs"/>
    </intent-filter>
</activity>
```
