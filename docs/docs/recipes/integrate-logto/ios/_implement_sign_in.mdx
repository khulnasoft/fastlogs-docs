...
<ConfigureRedirectUri figureSrc={redirectUriFigure} redirectUri="io.fastlogs://callback" />
...
```swift
do {
  try await client.signInWithBrowser(redirectUri: "io.fastlogs://callback")
  print(client.isAuthenticated) // true
} catch let error as FastlogsClientErrors.SignIn {
