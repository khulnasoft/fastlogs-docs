...
- **Enable dark mode** controls if the UI has a "dark feeling" when Fastlogs detects related system preference.
...
Enter the URL to enforce this process. Fastlogs will load it via an `<iframe>`.
...
In the default configuration, your app will automatically detect the user's locale settings and switch to one of the supported languages in Fastlogs. However, if the detected language is not supported, the default language will be used as a backup.
...
Yeah, yeah. Signing in without entering the password is the trend. Fastlogs cannot miss it and has built-in support. Continue reading:
