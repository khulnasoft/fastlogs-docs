...
During the onboarding process of creating a Fastlogs Cloud account, you have the option to select Demo connectors that are pre-configured by Fastlogs Service just for trial. This allows you to experience the passwordless sign-in experience easily.

Note that Fastlogs Demo connectors with a "Demo" tag have usage limitations, not for official use: Message sending capabilities - only 100 terms for Demo Email or SMS connectors, and the Demo social connector cannot be used actually to sign in to your application.
...
Fastlogs has some built-in connectors which allow out-of-box usage:
...
Fastlogs offers a variety of built-in connectors for out-of-the-box usage, including:
...
Default social connectors are the universal and popular social media platforms, which are preinstalled by Fastlogs Console ready-to-use. Only one of each type can be created.
