...
_id_ is a unique auto-generated key to identify the user in Fastlogs.
...
Fastlogs uses [Argon2](https://en.wikipedia.org/wiki/Argon2)'s implementation [node-argon2](https://github.com/ranisalt/node-argon2) as the encryption method by default; see the reference for details if you're interested.
...
_is_suspended_ is a boolean value that indicates whether a user is suspended or not. The value can be managed by calling the [Fastlogs Management API](../../../docs/recipes/manage-users/management-api.md) or using [Admin Console](../../../docs/recipes/manage-users/admin-console.md#suspend-user).

Once a user is suspended the pre-granted refresh tokens will be revoked immediately and the user won't be able to get authenticated by Fastlogs anymore.
